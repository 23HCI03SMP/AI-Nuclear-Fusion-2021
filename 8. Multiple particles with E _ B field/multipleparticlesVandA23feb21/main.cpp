...
int n_partd=1E6;
int n_parte=n_partd;
int n_output_part=1E10/n_partd; //maximum number of particles to output to file
...
//    double *t=new double[n_part[2]];
    double t=0;
    auto *pos0=new double[n_part[2]][3];
    auto *pos1=new double[n_part[2]][3];
    ...
    for(int p=0; p<2; p++)
    {
        ...
        for(int n=n_part[p]; n<n_part[p+1]; n++)
        {
            ...
            for(int c=0; c<3; c++)
            {
                pos0[n][c]=gsl_ran_flat(rng,poslp[c],poshp[c]);
                pos1[n][c]=pos0[n][c]+(gsl_ran_gaussian(rng,sigma)+v0[p][c])*dt[p];
            }
            q[n]=qs[p];
            m[n]=mp[p];
            nt[p]+=q[n];
...
//   E_file.precision(2);
    ...
    for (int i=0; i<ndatapoints; i++)
    {
        ...
        for (int p=0; p<2; p++)
        {
            ...
            for(int n=n_part[p]; n<n_part[p+1]; n++)
            {
                int i=(int)floor((pos1[n][0]-posl[0])/dd[0]+.5);
                int j=(int)floor((pos1[n][1]-posl[1])/dd[1]+.5);
                int k=(int)floor((pos1[n][2]-posl[2])/dd[2]+.5);
                if ((i>=0)&(i<n_space_divx)&(j>=0)&(j<n_space_divy)&(k>=0)&(k<n_space_divz))
                {
                    np[p][k][j][i]+=q[n]; //number of charge (in units of 1.6e-19 C  in each cell
                    nt[p]+=q[n];
                    //current density p=0 electron j=nev in each cell n in units 1.6e-19 C m/s
                    for(int c=0; c<3; c++)
                    {
                        currentj[p][k][j][i][c]+=q[n]*(pos1[n][c]-pos0[n][c])/dt[p];
                        It[p][i][c]+=q[n]*(pos1[n][c]-pos0[n][c])/dt[p];
                    }
                }
                else //if out of cells stop the particles from moving
                {
                    q[n]=0;
                    for(int c=0; c<3; c++) pos1[n][c]=pos0[n][c];
                }
            }
        }
// find current
        ...
        {
            o_file[0].open ("./out/e_"+to_string(i)+".csv");
            o_file[0] << std::scientific;
            o_file[0].precision(2);
            o_file[1].open ("./out/d_"+to_string(i)+".csv");
            o_file[1] << std::scientific;
            o_file[1].precision(2);
            o_file[0] <<"time, X, Y, Z, KE"<<endl;
            o_file[1] <<"time, X, Y, Z, KE"<<endl;
            ...
            for (int p=0; p<2; p++)
            {
                for(int n=n_part[p]; n<n_part[p+1]; n=n+ceil(n_parte/n_output_part))
                {
                    ...
                    for (int c=0; c<3; c++)
                    {
                        double dpos=(pos1[n][c]-pos0[n][c]);
                        dpos*=dpos;
                        dpos2+=dpos;
                    }
                    double KE=0.5*m[n]*(dpos2)/(e_charge_mass*dt[p]*dt[p]);
                    //in units of eV
                    o_file[p]<<t<<","<<pos0[n][0]<<","<<pos0[n][1]<<"," <<pos0[n][2] <<","<<KE<<endl;
                }
            }
            o_file[0].close();
            o_file[1].close();
        ...
        for (int p=0; p<2; p++)
        {
            ...
            for(int n=n_part[p]; n<n_part[p+1]; n++)
            {
                ...
                for (int jj=0; jj<ncalc[p]; jj++)
                {
                    ...
                    pyy=py*py;
                    pzz=pz*pz;
                    dx=2*pos1[n][0]-pos0[n][0]-pz*pos0[n][1]+py*pos0[n][2];
                    dy=2*pos1[n][1]-pos0[n][1]-px*pos0[n][2]+pz*pos0[n][0];
                    dz=2*pos1[n][2]-pos0[n][2]-py*pos0[n][0]+px*pos0[n][1];
                    for (int c=0; c<3; c++) pos0[n][c]=pos1[n][c];

                    r_determinant = 1.0/(1.0+pxx+pyy+pzz);
                    pos1[n][0]=r_determinant*((1.0 +pxx)*dx+(pz  +pxy)*dy+(pxz -py )*dz)+qdt2_2m*E[0];
                    pos1[n][1]=r_determinant*((pxy -pz )*dx+(1.0 +pyy)*dy+(pyz +px )*dz)+qdt2_2m*E[1];
                    pos1[n][2]=r_determinant*((pxz +py )*dx+(pyz -px )*dy+(1.0 +pzz)*dz)+qdt2_2m*E[2];
                    //stop particle
                    if(1)
                    {
                        for (int c=0; c<3; c++)
...
// print out pvd file for electric potential to make it easier to open the vti files in paraview
//   save_pvd("V", ndatapoints);
    std::chrono::steady_clock::time_point end = std::chrono::steady_clock::now();
    std::cerr << "Time difference = " <<(float)( std::chrono::duration_cast<std::chrono::milliseconds>(end - begin).count())/1000 << "[s]" << std::endl;
    return 0;
}
