...
void calc_pnn(float *Bx,float *By,float *Bz,float *Ex,float *Ey,float *Ez,float *pSos0x,float *pos0y,float *pos0z,float *pos1x,float *pos1y, float *pos1z,float *qdt_m, int n, cl::Context &context,cl::Device &default_device,cl::Program &program);
...
const int n_partd=n_space*n_space*n_space*64 ;
const int n_parte=n_partd;
const int n_output_part=min(n_partd,16384); //maximum number of particles to output to file
...
const float r_part_spart=1e7/n_partd;// ratio of particles per tracked "super" particle
